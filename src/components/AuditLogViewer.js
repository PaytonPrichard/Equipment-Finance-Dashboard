  ...
  const FilterSelect = ({ value, onChange, options, label }) => (
    <div className="flex flex-col gap-1">
      ...
      <select
        ...
        className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-[12px] text-gray-700 focus:outline-none focus:ring-1 focus:ring-gold-500/30 focus:border-gray-200 appearance-none cursor-pointer"
        style={{
  ...
  if (!can('audit.view')) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-gray-400 text-sm">You don't have permission to view the audit log.</div>
      </div>
    );
  }
    ...
    <div className="space-y-6 animate-fade-in-up">
        ...
        <div className="flex items-end gap-4 flex-wrap">
          ...
          {(actionFilter || entityFilter || dateRange) && (
            <button
              ...
              className="text-[11px] text-gray-400 hover:text-gray-700 transition-colors pb-2"
            >
              Clear filters
            </button>
          )}
      ...
      {loading && (
        <div className="glass-card rounded-2xl p-12 flex items-center justify-center">
          <div className="flex items-center gap-3">
            <div className="w-4 h-4 border-2 border-gray-300 border-t-gold-400 rounded-full animate-spin" />
            <span className="text-[12px] text-gray-400">Loading audit logs...</span>
          </div>
        </div>
      )}
      ...
      {!loading && !error && logs.length === 0 && (
        <div className="glass-card rounded-2xl p-12 text-center">
          ...
          <p className="text-sm text-gray-400 font-medium">No audit records found</p>
      ...
      {!loading && logs.length > 0 && (
        <div className="glass-card rounded-2xl overflow-hidden">
          {/* Table header */}
          <div className="grid grid-cols-[160px_140px_140px_140px_1fr] gap-2 px-5 py-3 border-b border-gray-200 bg-white/[0.01]">
              ...
              return (
                <div key={log.id}>
                  ...
                  >
                    ...
                    {/* User */}
                    <div className="text-[12px] text-gray-700 truncate" title={userName}>
                      {userName}
                    </div>
                    ...
                    <div className="text-[11px] font-mono">
                      ...
                      {log.entity_id && (
                        <span className="text-gray-400 ml-1" title={log.entity_id}>
          ...
          {hasMore && (
            <div className="px-5 py-4 border-t border-gray-200 text-center">
              <button
                ...
                className="px-5 py-2 rounded-lg text-[12px] font-medium text-gray-500 bg-gray-50 border border-gray-200 hover:bg-gray-100 hover:text-gray-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loadingMore ? (
                  <span className="flex items-center gap-2">
                    <span className="w-3 h-3 border-2 border-gray-300 border-t-gold-400 rounded-full animate-spin" />
                    Loading...
                  </span>
                ) : (
                  'Load More'
      ...
      {/* Legend */}
      <div className="flex items-center gap-6 text-[10px] text-gray-400 pt-2">
        {[
