        ...
        <div className="px-5 pb-5 animate-fade-in">
              ...
              <thead>
                <tr className="border-b border-gray-200">
                  {['Year', 'Principal', 'Interest', 'Total Payment', 'Ending Balance'].map((h) => (
                    <th key={h} className="text-[10px] font-bold text-gray-400 uppercase tracking-wider text-right pb-2 px-2 first:text-left">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {schedule.years.map((row) => {
                  ...
                  return (
                    <tr key={row.year} className="border-b border-white/[0.03] last:border-0">
                      <td className="py-2 px-2 text-[12px] text-gray-500 font-semibold">
                        Yr {row.year}
                      </td>
                      <td className="py-2 px-2 text-right">
                        <span className="font-mono text-[12px] text-gray-700">{formatCurrencyFull(row.principal)}</span>
                      </td>
                      <td className="py-2 px-2 text-right">
                        <span className="font-mono text-[12px] text-gray-500">{formatCurrencyFull(row.interest)}</span>
                      </td>
                      <td className="py-2 px-2 text-right">
                        <div>
                      ...
                      <td className="py-2 px-2 text-right">
                        <span className="font-mono text-[12px] text-gray-500">{formatCurrencyFull(row.endingBalance)}</span>
                      </td>
                    </tr>
                  );
                })}
          ...
          <div className="flex items-center gap-4 mt-3 text-[10px] text-gray-400">
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-1 rounded-full bg-gray-300" /> Principal portion
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-1 rounded-full bg-gray-50" /> Interest portion
            </span>
          </div>
        </div>
      )}
