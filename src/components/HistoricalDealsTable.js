  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest mr-1">Filter:</span>
        {['All', 'Performing', 'Paid Off', 'Watchlist', 'Defaulted'].map((s) => (
          <button
            key={s}
            onClick={() => setFilter(s)}
            className={`pill-btn px-3 py-1.5 rounded-lg text-[11px] font-medium ${filter === s ? 'pill-btn-active' : 'text-gray-400'}`}
          >
            {s}
        ...
        {filtered.map((deal) => {
          ...
          return (
            <div key={deal.id} className={`glass-card rounded-2xl overflow-hidden transition-all ${exp ? 'ring-1 ring-gold-500/20' : ''}`}>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-sm font-semibold text-gray-800 truncate">{deal.inputs.companyName}</span>
                    <span className="text-[10px] text-gray-400 font-mono flex-shrink-0">{deal.id}</span>
                  </div>
                ...
                <div className="flex-shrink-0" title={
                  ...
                  {deal.aligned === true ? (
                    ...
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="text-gray-600" strokeWidth="2">
                ...
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                  className={`text-gray-400 transition-transform flex-shrink-0 ${exp ? 'rotate-180' : ''}`} strokeWidth="2">
              ...
              {/* Expanded detail */}
              {exp && (
                <div className="px-5 pb-5 border-t border-gray-200 animate-slide-down">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <div key={x.l} className="bg-gray-50 rounded-xl p-2.5 text-center">
                            ...
                            <p className="font-mono font-semibold text-sm text-gray-800">{x.v}</p>
                          </div>
                        ))}
                      </div>
                      <div className={`rounded-xl p-3 border ${deal.rec.bgClass}`}>
                        ...
                        <p className="text-[11px] text-gray-500 mt-0.5">{deal.rec.detail}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {[
                          ...
                          <div key={x.l} className="bg-gray-50 rounded-xl p-2.5 text-center">
                            <p className="text-[10px] text-gray-400">{x.l}</p>
                            <p className={`font-mono font-semibold text-sm ${x.c || 'text-gray-800'}`}>{x.v}</p>
                          </div>
                        ))}
                      </div>
                      <div className="bg-gray-50 rounded-xl p-3">
                        <p className="text-[11px] text-gray-500 leading-relaxed">{deal.outcome.notes}</p>
                      </div>
                    </div>
                  </div>
                </div>
